...
│
├── Модуль перевірки та встановлення пакетів
│   ├── apt-get update
│   ├── openssh-server (якщо відсутній)
│   ├── nginx або apache2 (якщо жоден не встановлений — nginx)
│   ├── mysql-server або mariadb-server (якщо відсутній)
│   ├── vsftpd (за підтвердженням адміністратора)
...
Блок-схема нижче ілюструє алгоритм виконання скрипта з урахуванням механізмів відмовостійкості, описаних у підрозділі 3.3.

```mermaid
flowchart TD
    A(["▶ Запуск hardening.sh"]) --> B["set -Eeuo pipefail<br/>trap ERR → cleanup_on_error"]
    B --> C{"root-права?"}
    C -- Ні --> FAIL1(["✖ Вихід з помилкою"])
    C -- Так --> PKG_START

    subgraph PKG ["Модуль пакетів (§3.2)"]
        PKG_START["apt-get update"] --> PKG_SSH["openssh-server"]
        PKG_SSH --> PKG_WEB{"nginx або<br/>apache2 є?"}
        PKG_WEB -- Так --> PKG_MYSQL
        PKG_WEB -- Ні --> PKG_INST_WEB["встановити nginx"] --> PKG_MYSQL
        PKG_MYSQL{"mysql / mariadb є?"} -- Так --> PKG_FTP
        PKG_MYSQL -- Ні --> PKG_INST_DB["встановити mariadb-server"] --> PKG_FTP
        PKG_FTP{"vsftpd потрібен?"} -- Так --> PKG_INST_FTP["встановити vsftpd"] --> PKG_TOOLS
        PKG_FTP -- Ні --> PKG_TOOLS
        PKG_TOOLS["ufw · fail2ban · openssl"]
    end

    PKG_TOOLS --> D["Визначення середовища<br/>detect_webserver<br/>перевірка mysql / vsftpd"]
    D --> E["Резервне копіювання<br/>всіх конфігурацій"]
    ...
    subgraph SSH ["Модуль SSH (3.5)"]
        ...
        SSH_UP -- Ні --> SSH_WATCH["Watchdog відкочує<br/>через 60 с"]
        SSH_UP -- Так --> SSH_OK["✔ kill watchdog"]
    end

    SSH_OK --> WEB_START

    subgraph WEB ["Модуль веб-сервера (3.6)"]
        WEB_START{"nginx<br/>чи apache2?"} -- nginx --> WEB_NGINX["server_tokens off<br/>security-headers snippet<br/>autoindex off"]
        WEB_START -- apache2 --> WEB_APACHE["ServerTokens Prod<br/>Header always set ...<br/>Options -Indexes"]
        WEB_START -- відсутній --> WEB_SKIP["Пропустити модуль"]
        WEB_NGINX --> WEB_CHECK["nginx -t / apache2ctl -t"]
        WEB_APACHE --> WEB_CHECK
        WEB_CHECK --> WEB_RELOAD["reload веб-сервера"]
    end

    WEB_RELOAD --> MYSQL_START
    WEB_SKIP --> MYSQL_START

    subgraph MYSQL ["Модуль MySQL (3.7)"]
        MYSQL_START{"mysql<br/>встановлено?"} -- Так --> MYSQL_BIND["bind-address = 127.0.0.1"]
        MYSQL_START -- Ні --> MYSQL_SKIP["Пропустити модуль"]
        MYSQL_BIND --> MYSQL_SECURE["Видалити анонімних<br/>Кидати тестову БД<br/>Заблокувати remote root"]
        MYSQL_SECURE --> MYSQL_RELOAD["systemctl restart mysql"]
    end

    MYSQL_RELOAD --> FTP_START
    MYSQL_SKIP --> FTP_START

    subgraph FTP ["Модуль FTP (3.8)"]
        FTP_START{"vsftpd<br/>встановлено?"} -- Так --> FTP_TLS["ssl_enable=YES<br/>force_local_*_ssl=YES<br/>chroot_local_user=YES"]
        FTP_START -- Ні --> FTP_SKIP["Пропустити модуль"]
        FTP_TLS --> FTP_CERT{"Let's Encrypt<br/>сертифікат є?"}
        FTP_CERT -- Так --> FTP_LE["Використати /etc/letsencrypt"]
        FTP_CERT -- Ні --> FTP_SS["Згенерувати self-signed"]
        FTP_LE --> FTP_RELOAD["systemctl restart vsftpd"]
        FTP_SS --> FTP_RELOAD
    end

    FTP_RELOAD --> PERIM_START
    FTP_SKIP --> PERIM_START

    subgraph PERIM ["Модуль периметра (3.9)"]
        PERIM_START["ufw --force reset<br/>default deny incoming<br/>allow SSH / HTTP / HTTPS<br/>allow FTP якщо є"]
        PERIM_START --> F2B["Записати jail.local<br/>Fail2Ban для SSH / FTP / nginx"]
        F2B --> F2B_RELOAD["systemctl restart fail2ban"]
    end

    F2B_RELOAD --> REPORT["Генерація фінального звіту<br/>/var/log/hardening_*.log"]
    REPORT --> END(["✔ Завершено"])
```

*Рисунок 3.1 — Блок-схема алгоритму виконання `hardening.sh`. Перший крок — перевірка та встановлення відсутніх пакетів; кожен наступний модуль перевіряє наявність сервісу перед виконанням і пропускає його без помилки, якщо сервіс відсутній.*
...
## 3.10. Ідемпотентність скрипта

Ідемпотентність — властивість інструменту, при якій повторний запуск без зміни вхідних умов дає той самий результат, що і перший запуск, і не створює побічних ефектів. Для production-інструменту ця властивість є принциповою: адміністратор може запустити скрипт вдруге після зміни окремих налаштувань, після оновлення пакетів або просто для верифікації поточного стану — і система не деградує.

`hardening.sh` забезпечує ідемпотентність через три окремих патерни залежно від типу операції.

### Патерн 1: `sed` з підстановкою існуючого значення

Більшість директив конфігурації SSH, vsftpd і MySQL змінюються через `sed` із регулярним виразом, що знаходить рядок незалежно від поточного значення:

```bash
sed -i 's/^#*PermitRootLogin.*/PermitRootLogin no/' /etc/ssh/sshd_config
```

Регулярний вираз `^#*PermitRootLogin.*` знаходить рядок і в прокоментованому вигляді (`#PermitRootLogin yes`), і в активному (`PermitRootLogin yes`), і в уже правильному (`PermitRootLogin no`). При повторному запуску скрипт знайде вже правильне значення і замінить його на те саме — жодних дублікатів, жодних побічних ефектів.

### Патерн 2: `grep -q ... ||` перед додаванням

Декотрі операції не замінюють існуючий рядок, а додають новий — наприклад, якщо директива відсутня в конфігурації взагалі. Для таких випадків використовується grep-guard:

```bash
# MySQL: додати bind-address, якщо його немає
if grep -q "^bind-address" "$cfg"; then
    sed -i 's/^bind-address.*/bind-address = 127.0.0.1/' "$cfg"
else
    sed -i '/^\[mysqld\]/a bind-address = 127.0.0.1' "$cfg"
...
# FTP: додати директиву лише якщо її ще немає
grep -q 'allow_writeable_chroot' "$cfg" || \
    echo 'allow_writeable_chroot=YES' >> "$cfg"

# Nginx: підключити snippets лише якщо рядка ще немає
grep -q 'security-headers' /etc/nginx/sites-available/default || \
    sed -i '/listen 80/a\\tinclude snippets/security-headers.conf;' \
        /etc/nginx/sites-available/default
```

Без grep-guard виклик `sed -i '/http {/a\tserver_tokens off;'` при кожному запуску додавав би новий рядок `server_tokens off;` у блок, накопичуючи дублікати. Саме такі виклики є єдиним класом операцій, де наївна реалізація порушує ідемпотентність, і grep-guard усуває цю проблему.

### Патерн 3: повне перезаписування через `cat >`

Деякі конфігурації генеруються заново при кожному запуску — зокрема `/etc/fail2ban/jail.local` і файл Nginx snippets із security headers. Для них повний перезапис через `cat >` є природно ідемпотентним: результат завжди визначається лише вмістом heredoc, незалежно від кількості запусків.
...
Аналогічно `ufw --force reset` на початку модуля периметра скидає всі правила і застосовує їх заново — результат однаковий незалежно від того, перший це запуск чи п'ятий.

### Що відбувається при повторному запуску

Якщо запустити `hardening.sh` на вже обробленій системі:

| Операція | Результат повторного запуску |
|---|---|
| `sed` із підстановкою | Знаходить правильне значення, записує те саме — без змін |
| `grep -q \|\| append` | `grep` знаходить рядок, `append` не виконується |
| `cat >` heredoc | Перезаписує файл ідентичним вмістом |
| `ufw --force reset` | Скидає і негайно відновлює ті самі правила |
| Резервні копії | Створюється новий backup із новою міткою часу (очікувана поведінка) |
| `sshd -t`, `nginx -t` | Перевіряють незмінену конфігурацію — успіх |
| Dead Man's Switch | Запускається і скасовується після підтвердження — без відкату |

Єдиний побічний ефект повторного запуску — нові файли резервних копій із новою міткою часу. Це не є проблемою: вони не впливають на роботу системи і периодично можуть бути очищені адміністратором вручну.
